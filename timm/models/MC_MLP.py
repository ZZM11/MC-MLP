...
from .dct import dct_2d, idct_2d, dct1, dct
from timm.models.layers.mlp import GatedMlp_dctfc, Mlp_cat_nofc1, Mlp_dct, Mlp_dctfc, Mlp_dctmlp, Mlp_hadamamlp
# from .fft1d import fft
...
class MixerBlock(nn.Module):
    ...
    def forward(self, x):
        x = x + \
            self.drop_path(self.mlp_tokens(
                self.norm1(x).transpose(1, 2)).transpose(1, 2))
        x = x + self.drop_path(self.mlp_channels(self.norm2(x)))
        return x


class MixerBlock_dct(nn.Module):
    """ Residual Block w/ token mixing and channel MLPs
    Based on: 'MLP-Mixer: An all-MLP Architecture for Vision' - https://arxiv.org/abs/2105.01601
    """
    ...
    def __init__(
        ...
        self.norm2 = norm_layer(dim*2)
        self.mlp_channels2 = mlp_layer(
            dim*2, channels_dim, act_layer=act_layer, drop=drop)
        # self.norm1 = norm_layer(dim*2)
        self.norm1 = norm_layer(dim*2)
        self.mlp_channels1 = mlp_layer(
            dim*2, channels_dim, act_layer=act_layer, drop=drop)
        self.hadama = Hadmar()
        self.encode = dct_2d
        # self.mlp_channels2 = Mlp_dctmlp(
        #     dim, channels_dim, act_layer=act_layer, drop=drop)
        # self.mlp_channels1 = Mlp_hadamamlp(
        #     dim, channels_dim, act_layer=act_layer, drop=drop)
        # self.encode1 = dct1
    ...
    def forward(self, x):
        ...
        # y1 = y[:, 0:64, 0:64]
        # y = torch.cat((x, y), 2)
        # x = x + self.drop_path(self.mlp_channels1(self.norm1(y)))
        # x[:, 0:64, 0:64] = y1
        # -----------------------------------------------------------------
        # good
        # y = self.encode(x)
        ...
        # # y1 = y[:, 0:64, 0:64]
        # y = torch.cat((x, y), 2)
        # x = x + self.drop_path(self.mlp_channels2(self.norm2(y)))
        # ----------------------------------------------------------------------
        y = self.encode(x)
        # y1 = self.encode(x.transpose(1, 2))
        y = torch.cat((x, y), 2)
        x = x + self.drop_path(self.mlp_channels2(self.norm2(y)))
        # # ----------------------------------------------------------
        # # print(x.shape)
        ...
        # x = x.transpose(1, 2)
        # y = self.encode(x)
        # y1 = self.hadama(x.transpose(1, 2))
        y = torch.cat((x, y), 2)
        x = x + self.drop_path(self.mlp_channels1(self.norm1(y)))
        # x[:, 0:64, 0:64] = y1
        # x = self.norm4(x)
        # ------------------------------------------------------------
        # x = x + self.drop_path(self.mlp_channels2(x))
        # x = x + self.drop_path(self.mlp_channels1(x))
        return x


class MixerBlock_fft(nn.Module):
    """ Residual Block w/ token mixing and channel MLPs
    Based on: 'MLP-Mixer: An all-MLP Architecture for Vision' - https://arxiv.org/abs/2105.01601
    """
...
class MixerBlock_dct_nofc1(nn.Module):
    """ Residual Block w/ token mixing and channel MLPs
    Based on: 'MLP-Mixer: An all-MLP Architecture for Vision' - https://arxiv.org/abs/2105.01601
    """
    ...
    def __init__(
        ...
        self.mlp_channels = mlp_layer(
            dim*4, channels_dim, act_layer=act_layer, drop=drop)
        # self.hadama = Hadmar()
        self.encode = dct_2d
        self.pad = nn.ZeroPad2d(padding=(dim, dim, 0, 0))
    ...
    def forward(self, x):
        ...
        # print(y.shape)
        # print(x1.shape)
        x = x + self.drop_path(self.mlp_channels(self.norm2(x1)))
        return x
...
class SpatialGatingBlock_dctfc(nn.Module):
    """ Residual Block w/ Spatial Gating
...
class MlpMixer_dct(nn.Module):
    ...
    def __init__(
        ...
        self.head = nn.Linear(
            embed_dim, self.num_classes) if num_classes > 0 else nn.Identity()
    ...
    def init_weights(self, nlhb=False):
        ...
        named_apply(partial(_init_weights, head_bias=head_bias),
...
def _create_mixer_dct(variant, pretrained=False, **kwargs):
    if kwargs.get('features_only', None):
        raise RuntimeError(
            'features_only not implemented for MLP-Mixer models.')

    model = build_model_with_cfg(
        MlpMixer_dct, variant, pretrained,
...
# 3倍
@register_model
def MC_MLP_s(pretrained=False, **kwargs):
    """ Mixer-S/16 224x224
    Paper:  'MLP-Mixer: An all-MLP Architecture for Vision' - https://arxiv.org/abs/2105.01601
    # Flops:  6.09 GMac
    # Params: 24.07 M
    """
    model_args = dict(patch_size=14, num_blocks=15, embed_dim=256, **kwargs)
    model = _create_mixer_dct(
        'mixer_s16_224', pretrained=pretrained, **model_args)
    return model


@register_model
def MC_MLP_m(pretrained=False, **kwargs):
    """ Mixer-S/16 224x224
    Paper:  'MLP-Mixer: An all-MLP Architecture for Vision' - https://arxiv.org/abs/2105.01601
    Flops:  12.94 GMac
    Params: 50.89 M
    """
    model_args = dict(patch_size=14, num_blocks=32, embed_dim=256, **kwargs)
    model = _create_mixer_dct(
        'mixer_s16_224', pretrained=pretrained, **model_args)
    return model
...
@register_model
def MC_MLP_l(pretrained=False, **kwargs):
    """ Mixer-S/16 224x224
    Paper:  'MLP-Mixer: An all-MLP Architecture for Vision' - https://arxiv.org/abs/2105.01601
    Flops:  20.2 GMac
    Params: 79.28 M
    """
    model_args = dict(patch_size=14, num_blocks=50, embed_dim=256, **kwargs)
    model = _create_mixer_dct(
        'mixer_s16_224', pretrained=pretrained, **model_args)
    return model
